...
export const mysqlInsertByProductName = function(req, res) {
  const {
    productName,
    category,
    attributes: { video, shippingDate, included, specs },
    descriptions
  } = req.body;

  connection.query(
    `insert into attributes (productName, category, video, shippingDate, included, specs) values (?, ?, ?, ?, ?, ?);`,
    [
      productName,
      category,
      video,
    ...
    function(err, { insertId: productId }) {
      ...
      descriptions.forEach(description => {
        connection.query(
          `insert into descriptions (header, content, productId) values (?, ?, ?);`,
          [description.header, description.content, productId],
          function(err, { insertId: descriptionId }) {
            description.images.forEach(image => {
              connection.query(
                `insert into images (url, descriptionId) values (?, ?)`,
                [image, descriptionId]
              );
            });
          }
...
export const mysqlGetByProductName = function(req, res) {
  ...
  connection.query(query, function(err, results) {
    const descriptions = Object.values(
      results.reduce(function(accumulator, current) {
        if (accumulator[current.id])
          accumulator[current.id].images.push(current.url);
        else
    ...
    const overviewsHtml = renderToString(
      <Overviews initialState={initialState} />
    );
...
export const mysqlUpdateShipping = function(req, res) {
  ...
  connection.query(
    `update attributes set shippingDate = '${shippingDate}' where productName = '${productName}';`,
    function(err, result) {
...
export const mysqlUpdateDescriptions = function(req, res) {
  ...
  connection.query(
    `update descriptions set header = '${header}', content = '${content}' where id = ${descriptionId};`,
    function(err, result) {
      if (err) res.status(404);
      else res.status(200).send(result);
    }
...
export const mysqlDeleteDescription = function(req, res) {
  ...
  connection.query(`delete from descriptions where id = ${id}`, function(
    err,
    result
  ) {
    if (err) res.status(404);
      ...
      connection.query(
        `delete from images where descriptionId = ${id}`,
        function(err, result) {
